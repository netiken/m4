...
void write_flow_to_file(const char *filename, Flow *flows, int num_flows) {
    ...
    // Write each flow's details in the specified format
    for (int i = 0; i < num_flows; ++i) {
        fprintf(file, "%u %.2lf %f\n", flows[i].flowId, flows[i].size, flows[i].sldn);
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 4) { 
        ...
        data_flowsim_output = malloc(strlen(data_path) + strlen("/sldn_flowsim.txt") + 1); // +1 for null terminator
        strcpy(data_flowsim_output, data_path);
        strcat(data_flowsim_output, "/sldn_flowsim.txt");
    } else { error_usage(); }
    ...
    if (n_flows_fg>0){
        ...
        // }
        write_flow_to_file(data_flowsim_output, flows_fg, n_flows_fg);
        free(flows_fg);
    }
    // for(int i=0; i<n_feat_input; i++){
