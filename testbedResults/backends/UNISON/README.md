...
## Local Quickstart (Testbed Comparison)

The scripts in this tree are wired to the datasets that live in `new_scenarios/`. To regenerate the ns-3 sweeps that we compare against the real hardware:

```bash
cd UNISON
python3 run_sweep.py --ws 2,4,8,12,16 --rdma 1048584            # adjust window/RDMA sets as needed
python3 make_ns3output.py                                       # emits sweeps_4/*/ns3_output.txt

cd ../new_scenarios
python3 analyzev2.py --local-base expirements_4 --source ns3=../UNISON/sweeps_4:ns3_output.txt:2.0
```

`run_sweep.py` now uses project-relative paths and will rebuild the scratch target automatically. `make_ns3output.py` converts the raw `grouped_flows.txt` traces into the `[ud]/[rdma]` format consumed by the analyzer (it looks in `sweeps_4/` first and falls back to `sweeps/` for older runs).
