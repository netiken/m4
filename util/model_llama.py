...
class Transformer(nn.Module):
    ...
    def __init__(self, params: ModelArgs):
        ...
        self.tok_embeddings = nn.Linear(params.vocab_size, params.dim)
    ...
    def forward(self, tokens: torch.Tensor, targets: Optional[torch.Tensor] = None, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        #     print(f"tokens-{j}: {tokens[0,j,0]}, {tokens[0,j,-2]}")
        #     print(f"weights-{j}: {self.tok_embeddings.weight[0,0]}, {self.tok_embeddings.weight[-1,-1]}")
        #     print(f"bias-{j}: {self.tok_embeddings.bias[0]}, {self.tok_embeddings.bias[-1]}")
        #     print(f"tok_embeddings-{j}: {h[0,j,0]}, {h[0,j,-1]}")
        h = self.dropout(h)
