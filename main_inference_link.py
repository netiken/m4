...
class Inference:
    def __init__(self, model_config, training_config,checkpoint_path, lr, device='cuda:3'):
        ...
        for shard in np.arange(0, 5):
            for nflows in [2000]:
                for nhosts in [21]:
                    ...
                    print(f"Finished workload={shard}. fct shape: {fct_tmp.shape}, sldn shape: {sldn_tmp.shape}")
                    fct.append(fct_tmp)
