...
void hrd_connect_qp(struct hrd_ctrl_blk* cb, int n,
  ...
  conn_attr.path_mtu = IBV_MTU_1024; //IBV_MTU_4096;
  conn_attr.dest_qp_num = remote_qp_attr->qpn;
  conn_attr.rq_psn = HRD_DEFAULT_PSN;

  /* RoCE uses global routing header. Build GRH with remote GID. */
  conn_attr.ah_attr.is_global = 1;
  conn_attr.ah_attr.dlid = 0; /* RoCE does not use LID */
  ...
  conn_attr.ah_attr.grh.dgid = remote_qp_attr->gid;
  {
    ...
    conn_attr.ah_attr.grh.sgid_index = gid_index; /* Use configured GID index */
  }
  ...
  if (!cb->use_uc) {
    /* Cap max_dest_rd_atomic to device capability */
    struct ibv_device_attr dev_attr;
    ibv_query_device(cb->ctx, &dev_attr);
    conn_attr.max_dest_rd_atomic = dev_attr.max_qp_rd_atom < 16 ? dev_attr.max_qp_rd_atom : 16;
    conn_attr.min_rnr_timer = 12;
    rtr_flags |= IBV_QP_MAX_DEST_RD_ATOMIC | IBV_QP_MIN_RNR_TIMER;
  }

  int status = ibv_modify_qp(cb->conn_qp[n], &conn_attr, rtr_flags);
  if (status) {
    fprintf(stderr, "HRD: Failed to modify QP to RTR: %d\n", status);
    assert(false);
  }
  ...
  if (!cb->use_uc) {
    struct ibv_device_attr dev_attr2;
    ibv_query_device(cb->ctx, &dev_attr2);
    conn_attr.max_rd_atomic = dev_attr2.max_qp_rd_atom < 16 ? dev_attr2.max_qp_rd_atom : 16;
    conn_attr.max_dest_rd_atomic = conn_attr.max_rd_atomic;
  ...
  conn_attr.ah_attr.src_path_bits = 0;
  conn_attr.ah_attr.port_num = cb->dev_port_id; /* Local port! */
  //conn_attr.ah_attr.grh.sgid_index = 3;
  ...
  int status = ibv_exp_modify_qp(cb->conn_qp[n], &conn_attr, rtr_flags);
  if (status) {
    fprintf(stderr, "HRD: Failed to modifyexp QP to RTR: %d\n", status);
    assert(false);
  }
...
void hrd_publish_conn_qp(struct hrd_ctrl_blk* cb, int n, const char* qp_name) {
  ...
  /* Query local GID for RoCE */
  {
    int gid_index = 0;
    char* env = getenv("HRD_ROCE_GID_INDEX");
    if (env != NULL) gid_index = atoi(env);
    if (ibv_query_gid(cb->ctx, cb->dev_port_id, gid_index, &qp_attr.gid)) {
      fprintf(stderr, "HRD: ibv_query_gid failed\n");
      assert(false);
    }
  }

  qp_attr.lid = 0; /* Not used in RoCE */
