...
void* run_client(void* arg) {
  ...
  char log_path[256];
  sprintf(log_path, "/home/zabreyko/rdma_bench/herd/log_%d.txt", clt_gid);
  ...
  /* Event logging only; per-flow timing removed */
  ...
  while (1) {
    /* Enforce per-client request cap if configured */
    if (HERD_CLIENT_REQ_LIMIT >= 0 && nb_tx >= HERD_CLIENT_REQ_LIMIT) {
      printf("main: Client %d reached request cap (%ld), exiting.\n", clt_gid,
             (long)HERD_CLIENT_REQ_LIMIT);
      fflush(stdout);
      if (log_fp != NULL) {
        fflush(log_fp);
        fclose(log_fp);
    ...
    if (nb_tx % WINDOW_SIZE == 0 && nb_tx > 0) {
      ...
      /* Log response receive events */
      for (int k = 0; k < WINDOW_SIZE; k++) {
        uint32_t imm = wc[k].imm_data;
        uint16_t wrkr = (uint16_t)(imm >> 16);
        uint16_t slot = (uint16_t)(imm & 0xFFFF);
        uint32_t resp_bytes = wc[k].byte_len;

        struct timespec ts_now;
        clock_gettime(CLOCK_REALTIME, &ts_now);
        uint64_t ts_ns = (uint64_t)ts_now.tv_sec * 1000000000ULL + (uint64_t)ts_now.tv_nsec;
        fprintf(log_fp,
                "event=resp_recv ts_ns=%" PRIu64 " clt=%d wrkr=%u slot=%u size=%u src=worker:%u dst=client:%d\n",
                ts_ns, clt_gid, wrkr, slot, resp_bytes, wrkr, clt_gid);
      }
    }
    ...
    *(uint128*)req_buf = CityHash128((char*)&key_arr[key_i], 4);
    ...
    if (is_update) {
      /* Derive variable length deterministically from key and fill value */
      uint64_t part0 = ((uint64_t*)req_buf)[0];
      uint64_t part1 = ((uint64_t*)req_buf)[1];
      uint8_t vlen = herd_val_len_from_key_parts(part0, part1);
      req_buf->val_len = vlen;
      uint8_t val = (uint8_t)(part1 & 0xff);
      memset(req_buf->value, val, vlen);
    } else {
      req_buf->val_len = (uint8_t)-1; /* ignored for GET */
    }

    /* Forge the RDMA work request */
    if (is_update) {
      /* 16B key + 1B opcode + 1B len + value */
      sgl.length = 16 + 1 + 1 + req_buf->val_len;
    } else {
      sgl.length = HERD_GET_REQ_SIZE;
    }
    ...
    CPE(ret, "ibv_post_send error", ret);
    {
      ...
      uint64_t ts_ns = (uint64_t)ts_now.tv_sec * 1000000000ULL + (uint64_t)ts_now.tv_nsec;
      fprintf(log_fp,
              "event=req_send ts_ns=%" PRIu64 " clt=%d wrkr=%d slot=%d size=%u src=client:%d dst=worker:%d\n",
              ts_ns, clt_gid, wn, curr_slot, sgl.length, clt_gid, wn);
    }
